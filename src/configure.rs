...
const CHEAT_SHEET_FILENAME: &str = "notionsmith-courses.txt";
...
fn write_cheat_sheet(watch_dir: &Path, lines: &[String]) -> io::Result<()> {
    let mut content = String::from("Notionsmith course prefixes\n\nName files as: [PREFIX]_[Topic]_[Date].pdf\n\n");
    for line in lines {
...
const COURSE_BLOCK_MARKER: &str = "# Course prefixes (generated by `notionsmith configure`)";

/// Writes the confirmed prefix mapping back to the env file: existing
/// `NOTEIN_COURSE_` lines (and this function's own marker comment above them) are
/// dropped and replaced with a fresh block, everything else in the file is left
/// untouched.
fn write_course_map(env_path: &Path, rows: &[CourseRow]) -> io::Result<()> {
    ...
    let mut kept_lines: Vec<&str> = existing
        ...
        .filter(|line| {
            ...
            !trimmed.starts_with("NOTEIN_COURSE_") && trimmed != COURSE_BLOCK_MARKER
        })
        .collect();
    ...
    output.push('\n');
    output.push_str(COURSE_BLOCK_MARKER);
    output.push('\n');
    for row in rows {
...
fn draw(frame: &mut ratatui::Frame, rows: &[CourseRow], selected: usize, mode: &Mode, status: &str) {
    ...
    let preview = Paragraph::new(
        format!("Name Notein exports as [PREFIX]_[Topic]_[Date].pdf, e.g. {}_Topic_{}.pdf", preview_prefix, today)
    ).block(Block::default().borders(Borders::ALL).title("Naming convention"));
    ...
    let list = List::new(items)
        .block(Block::default().borders(Borders::ALL).title("Courses (Notion Course property)"))
        .highlight_style(Style::default().add_modifier(Modifier::REVERSED));
    frame.render_stateful_widget(list, chunks[1], &mut list_state);

    let help = match mode {
        Mode::Browsing => "up/down select   Enter edit prefix   s save & quit   q quit without saving",
        Mode::Editing(_) => "type to change   Enter confirm   Esc cancel",
    };
...
/// Runs the interactive `configure` TUI: fetch the Notion course select options,
                ...
                match key.code {
                    ...
                    KeyCode::Char('s') => {
                        ...
                        if !duplicates.is_empty() {
                            status = format!("⚠ Duplicate prefixes, fix first: {}. Not saved.", duplicates.join(", "));
                            continue;
                        }

                        write_course_map(&env_path, &rows)?;
                        let lines = reference_lines(&rows);
                        let mut messages = vec![format!("Saved to {}.", env_path.display())];

                        match
                            notion::upsert_reference_page(client, notion_token, database_id, title_property, &lines).await
                        {
                            Ok(()) =>
                                messages.push(format!("Updated reference page '{}' in Notion.", notion::REFERENCE_PAGE_TITLE)),
                            Err(e) => messages.push(format!("Notion reference page failed: {}.", e)),
                        }

                        if let Err(e) = write_cheat_sheet(watch_dir, &lines) {
                            messages.push(format!("Local cheat sheet failed: {}.", e));
                        }
